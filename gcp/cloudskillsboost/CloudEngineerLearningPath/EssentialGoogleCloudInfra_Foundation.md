...
## 实验：[使用 Google Cloud 控制台和 Cloud Shell](../labs/393747.md)

## 实验：[Google Cloud 础架构预览](../labs/393750.md)
...
> FQDN（Fully Qualified Domain Name）是一个用于唯一标识互联网上特定主机的域名。
> FQDN 包含了从顶级域名一直到主机名的所有标签，以完整的层级结构描述了主机在域名系统（DNS）中的位置。
...
## 实验：[VPC 网络 - VPC Networking](../labs/393766.md)
...
## 实验：[实现专用 Google 访问通道和 Cloud NAT](../labs/393770.md)
...
Google Cloud 提供了多种不同的选项来保持 Compute Engine 资源的低价：
...
## 特殊计算配置
![可抢占式 VM](../images/preemptible-gce.png)

![Spot VM](../images/spot-vm.png)

![单组户](../images/sole-tenant-vm.png)  
...
![机密虚拟机](../images/confidential-vm.png)  
...
如果您在同一区域内移动实例，则可以使用 gcloud compute instance move 命令自动执行移动。要移动 VM，您必须关闭 VM，将其移动到目标专区或区域，然后重新启动。移动 VM 后，更新对原始资源的所有引用，例如指向早期 VM 的任何目标 VM 或目标池。在移动过程中，虚拟机和磁盘的一些服务器生成的属性会发生变化。  
...
---
另一个常见的 Compute Engine 操作是调整永久性磁盘的大小。增加存储容量的额外好处是提高 I/O 性能。这可以在磁盘连接到正在运行的虚拟机时实现，而无需创建快照。  
虽然您可以增大磁盘的大小，但永远无法缩小它们。  
![调整永久磁盘大小](../images/resize-persistent-disk.png)

## 实验：[使用虚拟机](../labs/393788.md)
