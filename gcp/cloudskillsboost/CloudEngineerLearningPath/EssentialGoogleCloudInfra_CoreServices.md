...
## 实验：[探索 Google Cloud IAM](../labs/386456.md)
...
随着您对 Google Cloud 的使用不断扩大，您的配额可能会相应增加。如果您预计使用量即将显着增加，您可以从 Google Cloud 控制台的配额页面主动请求配额调整。
...
## 实验：[通过 BigQuery 查看结算数据](../labs/386485.md)
...
## 实验：[资源监控](../labs/386493.md)

## logging
Cloud Logging 允许您存储、搜索、分析、监控来自 Google Cloud 和 AWS 的日志数据和事件并发出警报。它是一项完全托管的服务，可以大规模执行，可以从数千个虚拟机中获取应用程序和系统日志数据。

日志记录包括日志存储、称为 Logs Explorer 的用户界面以及用于以编程方式管理日志的 API。该服务允许您读取和写入日志条目、搜索和过滤日志以及创建基于日志的指标。

## Error Reporting
错误报告对正在运行的云服务中的错误进行计数、分析和聚合。集中式错误管理界面显示具有排序和过滤功能的结果，您甚至可以在检测到新错误时设置实时通知。

## Cloud Trace
Cloud Trace 是一个分布式跟踪系统，可从您的应用收集延迟数据并将其显示在 Google Cloud 控制台中。您可以跟踪请求如何在应用程序中传播，并获得详细的近乎实时的性能见解。

## Profiler
性能不佳的代码每天都会增加应用程序和 Web 服务的延迟和成本。 Cloud Profiler 持续分析跨应用程序执行的 CPU 或内存密集型函数的性能。

## 实验：[错误报告和调试](../labs/386499.md)

## 模块回顾
在本模块中，我向您概述了 Google Cloud 的操作套件及其监控、日志记录、错误报告、故障跟踪和分析功能。

将所有这些集成到 Google Cloud 中后，您就可以操作和维护应用程序，这称为站点可靠性工程或 SRE。
